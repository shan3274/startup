...
const Header = () => {
  ...
  console.log("user data mainpage", users.name, users.email);
  console.log(users);
      ...
      <div className="w-full h-screen">
        <nav className="w-full h-[5rem] flex items-center justify-around">
          ...
          <div className="sm:w-[10%] relative sm:right-0 right-8 flex items-center justify-start pl-10  text-[25px]">
            {menuOpen ? (
              <AiOutlineClose onClick={() => setMenuOpen(false)} className="text-[20px] sm:text-[25px]" />
            ) : (
              <AiOutlineMenu onClick={() => setMenuOpen(true)} className="text-[20px] sm:text-[25px]"/>
            )}
          </div>
          {/* logo */}
          <div className="w-[10%] relative sm:right-0 right-6">
            <svg
          ...
          <div className="w-[50%] h-full flex items-center justify-center">
            <input
              ...
              placeholder="Search"
              className="w-[80%] sm:h-[50%] border border-black rounded-3xl px-5"
            />

            <button className="relative sm:text-[25px] sm:right-[5%] right-[10%]">
              <FcSearch />
            </button>
          </div>
          {/* authentication button */}
          <div className="w-[20%] flex gap-[10px] justify-center items-center">
            {userEmail != null ? (
              <>
                <button onClick={() => setLoggedin(true)}>
                  <BiUserCircle
                  className="sm:text-[42px] text-[30px]"
                  />
                </button>
              </>
            ) : (
              <button
                className="w-[130px] border sm:h-[35px] relative sm:right-0 right-2 sm:text-[16px] text-[12px] h-[25px]  rounded-xl text-white bg-gray-400 hover:bg-gray-500 transition-colors"
        ...
        {/* Search bar modal */}
        {openModal && <Search closeModal={setOpenModal} />}
        {/* Login and signuo Modal */}
