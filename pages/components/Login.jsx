...
const Login = ({ closeModal }) => {
  ...
  // console.log("user data", users.name, users.email);
  ...
  useEffect(() => {
    if (user != null) {
      onAuthStateChanged(auth, (currentUser) => {
        dispatch(
          login({ name: "shan khan", age: 23, email: currentUser.email })
        );
        setUser(currentUser);
      });
      // dispatch(login({ name: "shan khan", age: 23, email: user.email }));
    }
  }, [user]);
  return (
    <div>
      <div className="w-full h-screen flex items-center justify-center absolute top-0 bg-trans backdrop-blur-sm">
        <div className="sm:w-[40%] sm:h-[70%] flex flex-col items-center p-10 justify-start gap-10 bg-white backdrop-blur-xl border rounded-lg drop-shadow-sm">
          ...
          <input
            type="text"
            className="border w-[80%] h-[40px] rounded-xl border-black px-5 hover:scale-[1.05] hover:drop-shadow-xl transition-[1s] bg-transparent"
            placeholder="Email"
            value={email}
      ...
      {signup && <Signup closeModal={setSingup} closeLogin={closeModal} />}
    </div>
  );
};
